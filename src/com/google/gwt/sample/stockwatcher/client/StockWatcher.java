...
public class StockWatcher implements EntryPoint {
   ...
   private void addStock() {
      ...
      if (symbol.length() <= 0) {
         Window.alert("Empty stock symabol are forbidden.");
      } else if (symbol.length() > MAX_LENGTH) {
         Window.alert("'" + symbol + "' is too long, max length is " + MAX_LENGTH);
      } else if (!symbol.matches("^[0-9A-Z\\.]{1,10}$")) {
         // Stock code must contain only numbers, letters,or dots.
